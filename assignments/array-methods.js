...
console.log("Challenge 1: Use .forEach():", fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = runners.map(runner=>runner.first_name.toUpperCase());
console.log("Challenge 2: Use .map():", allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = runners.filter(runner => runner.shirt_size === "L");
console.log("Challenge 3: Use .filter();",largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce((total, runner) => total += runner.donation, 0);
console.log("Challenge 4: Use .reduce()",ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1
//We Want to know what companies had employees who participated and also have that list in Alphabetical order and no repeats
let companiesRepresented = runners
    .map(runner=>runner.company_name)
    .filter((item,index) => runners.map(runner=>runner.company_name).indexOf(item) === index)
    .sort();

console.log("Challenge 5, Problem 1: What Companies Had Employees Participate:", companiesRepresented);

// Problem 2
//How many different t-shirt sizes are there? 
let tShirtTypes = runners.map(runner => runner.shirt_size)
                         .filter((item, index) => runners.map(runner => runner.shirt_size).indexOf(item) === index)
                         .length
console.log("Challenge 5, Problem 2: How Many Different T-Shirt Sizes Are There:", tShirtTypes);


// Problem 3
// So, let's just get everybody's email address. That's not like a GDPR violation or anything. 
let emailAddresses = runners.map(runner => runner.email)
console.log("Challenge 5, Problem 3: Everybody's Email Addresses:", emailAddresses);